    ...
    namespace Sqlite {
        ...
        private:
            ...
            private:
                ...
                typedef void(EventHandler)(const char* owner, IARM_EventId_t eventId, void* data, size_t len);

            public:
                IARMHandler()
                    : _clockSynced(false)
                    , _handler([&](const char* owner, IARM_EventId_t eventId, void* data, size_t len) {
                        ...
                        if (sysEventData->data.systemStates.stateId == IARM_BUS_SYSMGR_SYSSTATE_TIME_SOURCE) {
                            ...
                            _clockSynced = sysEventData->data.systemStates.state;
                        }
                    })
                {
                    ...
                    rc = IARM_Bus_RegisterEventHandler(IARM_BUS_SYSMGR_NAME, IARM_BUS_SYSMGR_EVENT_SYSTEMSTATE, *(_handler.template target<EventHandler*>()));
                    if (rc != IARM_RESULT_SUCCESS) {
                ...
                ~IARMHandler()
                {
                    auto rc = IARM_Bus_RemoveEventHandler(IARM_BUS_SYSMGR_NAME, IARM_BUS_SYSMGR_EVENT_SYSTEMSTATE, *(_handler.template target<EventHandler*>()));
                    if (rc != IARM_RESULT_SUCCESS) {
                        TRACE(Trace::Error, (_T("Sqlite IARM remove error %d"), rc));
                    }
                }
                inline bool ClockSynced() const { return _clockSynced; };

            private:
                std::atomic<bool> _clockSynced;
                std::function<EventHandler> _handler;
            };
